/**
 * Typed Socket.IO event maps for the Swibber frontend client.
 * Mirrors backend/src/socket/types.ts — keep in sync.
 */
...
export interface ClientToServerEvents {
  ...
  // Ride
  driver_accept_ride: (data: { rideId: string; driverId: string }) => void;
  ...
  verify_ride_otp:    (data: { rideId: string; otp: string }) => void;
  driver_location:    (data: { rideId: string; lat: number; lng: number; heading?: number }) => void;
  request_eta_update: (data: { rideId: string; driverLat: number; driverLng: number }) => void;
  ...
  cancel_ride_socket: (data: { rideId: string; reason?: string }) => void;

  // Parcel
  rider_accept_parcel:   (data: { parcelId: string; riderId: string }) => void;
  parcel_status_update:  (data: { parcelId: string; status: ParcelStatus }) => void;
  parcel_location_update: (data: { parcelId: string; lat: number; lng: number; heading?: number; speed?: number }) => void;

  'parcel:subscribe':   (data: { parcelId: string }) => void;
  'parcel:unsubscribe': (data: { parcelId: string }) => void;

  // Food
  update_order_status:   (data: { orderId: string; status: OrderStatus; riderId?: string }) => void;
  rider_location_update: (data: { orderId: string; lat: number; lng: number }) => void;
